		...
		$query=mysqli_query($con,"select password from user where user_id='$id'")or die(mysqli_error());
			$row=mysqli_fetch_array($query);
				...
				if ($passold==$pass)
				{
					if ($password<>"")
					{
						mysqli_query($con,"update user set name='$name', lname='$lname', username='$username', password='$passnew' where user_id='$id'")or die(mysqli_error($con));
					}
					else
					{
						mysqli_query($con,"update user set name='$name', lname='$lname', username='$username' where user_id='$id'")or die(mysqli_error($con));
					}
