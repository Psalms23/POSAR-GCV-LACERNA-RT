...
include('../dist/includes/dbcon.php');
	...
	$amount_due = number_format($_POST['amount_due']);
	...
	$check_no=$_POST['check_no'];
		...
		$tendered = $_POST['check_tendered'];
		$change = number_format($_POST['change']);

		mysqli_query($con,"INSERT INTO sales(cust_id,user_id,discount,amount_due,total,date_added,modeofpayment,cash_tendered,cash_change,branch_id, check_no) 
	VALUES('$cid','$id','$discount','$amount_due','$total','$date','check','$tendered','$change','$branch','$check_no')")or die(mysqli_error($con));
	...
	$query=mysqli_query($con,"select * from temp_trans where branch_id='$branch'")or die(mysqli_error($con));
		...
		$query1=mysqli_query($con,"SELECT or_no FROM payment NATURAL JOIN sales WHERE modeofpayment =  'check' ORDER BY payment_id DESC LIMIT 0 , 1")or die(mysqli_error($con));
