...
include('../dist/includes/dbcon.php');
	...
	$qty = $_POST['qty'];
			...
			if ($row['prod_qty'] == 0 || $row['prod_qty'] < $qty) {
				echo "<script>alert('Insufficient Quantity of Product'); document.location='cash_transaction.php?cid=$cid';</script>";
			}else if($qty <= 0 ){
				echo "<script>alert('Invalid Quantity'); document.location='cash_transaction.php?cid=$cid'</script>";
			}else{
